...
const Tile = ({ isBomb, revealed, onPress }) => {
  return (
    <TouchableOpacity
      ...
      onPress={onPress}
      disabled={revealed}>
      {revealed && (isBomb ?  <Text style={Styles.tileText}>100</Text> : <Text style={Styles.tileText}>100</Text>)}
    </TouchableOpacity>
  );
};
...
export default function Page2() {
  ...
  const NUM_BOMBS = 1; // this is how many bombs
  ...
  const getScore = () => {
    // 100 points when tile is reveled 
    return revealedTiles.filter((revealed, index) => revealed && !bombPositions.has(index)).length * 100;
  };
