...
const Tile = ({ isBomb, revealed, onPress }) => {
  return (
    <TouchableOpacity
      ...
      onPress={onPress}
      disabled={revealed}>
      {revealed && (isBomb ?  <Text style={Styles.tileText}>1000</Text> : <Text style={Styles.tileText}>1000</Text>)}
    </TouchableOpacity>
  );
};
...
export default function ten() {
  ...
  const NUM_BOMBS = 10; // this is how many bombs
  ...
  const getScore = () => {
    // 1000 points when tile is reveled 
    return revealedTiles.filter((revealed, index) => revealed && !bombPositions.has(index)).length * 1000;
  };
