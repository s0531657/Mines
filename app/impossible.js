...
const Tile = ({ isBomb, revealed, onPress }) => {
  return (
    <TouchableOpacity
      ...
      onPress={onPress}
      disabled={revealed}>
      {revealed && (isBomb ?  <Text style={Styles.tileText}>5000</Text> : <Text style={Styles.tileText}>5000</Text>)}
    </TouchableOpacity>
  );
};
...
export default function impossible() {
  ...
  const NUM_BOMBS = 24; // this is how many bombs
 ...
 // randomly put the bomb in the grid
  ...
  const getScore = () => {
    // 5000 points when tile is reveled 
    return revealedTiles.filter((revealed, index) => revealed && !bombPositions.has(index)).length * 5000;
  };
